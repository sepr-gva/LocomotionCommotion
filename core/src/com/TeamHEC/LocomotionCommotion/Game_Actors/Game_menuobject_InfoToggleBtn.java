...
public class Game_menuobject_InfoToggleBtn extends Actor {
	...
	public static float actorX = 1038 ,actorY = 38;
	...
	public void act(float delta){
		if(started){
			if (Game_MapManager.infoVisible== false)
			{
				...
				GameScreen.getStage().getActors().get(Game_MapManager.ticketActors).setVisible(true);

			}
			else
			{	Game_MapManager.infoVisible= false;
			GameScreen.getStage().getActors().get(Game_MapManager.ticketActors).setVisible(false);
