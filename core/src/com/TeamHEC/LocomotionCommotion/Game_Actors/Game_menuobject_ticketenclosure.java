...
public class Game_menuobject_ticketenclosure extends Actor {

	public static Texture texture = Game_TextureManager.getInstance().game_menuobject_ticketenclosure; // reuse the new game back btn texture
	public static float actorX = -1 ,actorY = Game_menuobject_TopBar.actorY-texture.getHeight() +11;
	public boolean started = false;
	

	public Game_menuobject_ticketenclosure(){
		setBounds(actorX,actorY,texture.getWidth(),texture.getHeight());
		addListener(new InputListener(){
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				((Game_menuobject_ticketenclosure)event.getTarget()).started = true;
				return true;
			}
		});
