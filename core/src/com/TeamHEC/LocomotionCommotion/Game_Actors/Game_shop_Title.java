...
public class Game_shop_Title extends Actor {

	public static Texture texture = Game_TextureManager.getInstance().game_shop_title; // reuse the new game back btn texture
	public static float actorX = 170 ,actorY = 820;
	public boolean started = false;
	

	public Game_shop_Title(){
		setBounds(actorX,actorY,texture.getWidth(),texture.getHeight());
		addListener(new InputListener(){
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				((Game_shop_Title)event.getTarget()).started = true;
				return true;
			}
		});
