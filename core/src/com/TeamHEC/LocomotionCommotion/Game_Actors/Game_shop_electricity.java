...
public class Game_shop_electricity {
	...
	public int quantity, cost, posx=1100, posy=450;
	public static LabelStyle style;
	public Game_shop_electricity(){
		this.actors = new ArrayList<Actor>();
		OilItem coalitem = new OilItem();
	...
	public class OilItem extends Actor{
		Texture texture = Game_TextureManager.game_shop_electricityitem; // reuse the new game back btn texture
		...
		public OilItem(){
			...
			addListener(new InputListener(){
				public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
					((OilItem)event.getTarget()).started = true;
					return true;
				}
			});
