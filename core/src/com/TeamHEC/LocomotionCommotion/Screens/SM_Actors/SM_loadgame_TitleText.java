 ...
 * 
 * This is an Actor FOR THE TITLE TO THE LOAD GAME PAGE- meaning it's given texture is displayed on the stage and actions (acts) can be performed.
 ...
 * 				Action- None
 */
public class SM_loadgame_TitleText extends Actor {

	Texture texture = SM_TextureManager.sm_loadgame_Title;
	float actorX = 1250+ 350 ,actorY = 665;
	...
	public SM_loadgame_TitleText(){
		...
		addListener(new InputListener(){
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				((SM_loadgame_TitleText)event.getTarget()).started = true;
				return true;
			}
		});
	}
